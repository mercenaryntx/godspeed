...
namespace Neurotoxin.Godspeed.Core.Net {
    ...
    public class FtpClient : IFtpClient {
        ...
        public event EventHandler<FtpGetListingEventArgs> GetListingDataReceived;
        ...
        private void OnGetListingDataReceived(int size)
        {
            var handler = GetListingDataReceived;
            if (handler != null) handler.Invoke(this, new FtpGetListingEventArgs(size));
        }
                ...
                try
                {
                    if (IsConnected)
                    {
                        ...
                        if (stream.CommandStatus.Type == FtpResponseType.PositivePreliminary)
                        {
                            ...
                            if (!reply.Success)
                            {
                                if (reply.Code != "426") throw new FtpCommandException(reply);
                                ClearStaleData();
                            }
        ...
        {
            if (m_stream.IsConnected && m_stream.SocketDataAvailable > 0)
            {
                var buf = new byte[m_stream.SocketDataAvailable];
                m_stream.RawSocketRead(buf);
                var data = Encoding.GetString(buf);
                //Debugger.Break();
            }
        }
        ...
        private FtpListItem[] _getListingCache;
        private string _getListingDirname;
        ...
        public FtpListItem[] GetListing(string path, FtpListOption options) {
            ...
            if (_getListingDirname == path && _getListingCache != null)
                return _getListingCache;
                        ...
                        using (var ms = new MemoryStream())
                        {
                            ...
                            do
                            {
                                ...
                                while ((bufferSize = stream.Read(buffer, 0, buffer.Length)) > 0)
                                {
                                    ...
                                    size += bufferSize;
                                    OnGetListingDataReceived(size);
                                    sw.Restart();
                                }
                                if (m_stream.SocketDataAvailable > 0) reply = m_stream.ReadLine();
                                if (reply == null && sw.ElapsedMilliseconds > DataConnectionReadTimeout)
            ...
            _getListingCache = lst.ToArray();
            _getListingDirname = path;
        ...
        {
            _getListingCache = null;
            _getListingDirname = null;
        }
